...
- `.mp4.zst` / `.mkv.zst` 是纯 zstd 流 → 用 `7z x`（Linux 装 `sudo pacman -S 7zip`）；Win11 自带 `tar.exe` 会报 Unrecognized archive format。只有 `.tar.zst` 才能 `tar -xf`。
...
有规则的立即处理；无规则/超预算的写「处理询问文档」（编号 C1/C2…，每项给方案选项）等用户逐条回复。Linux 路径 `~/处理询问.md`（别放下载目标盘，避免被空间守卫波及）。每批结束更新尾部「处理进度」：完成项、各盘 free（`df -h <挂载点1> <挂载点2>`）、卡点原因。
...
- 已安装并登录 BaiduPCS-Go（见技能 `baidupcs-cli`）；`7z`（Linux `extra/7zip`，本机已装）与 `hub` 工具。
- Windows 对应写法：`(Get-PSDrive E).Free/1GB -lt 15` 判空间、`Clear-RecycleBin -DriveLetter X -Force`、`Stop-Process BaiduPCS-Go -Force`；PowerShell 逻辑一律写 `.ps1` 跑（5.1 用 `-File` 读无 BOM 中文会炸引号）。
