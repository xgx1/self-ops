...
- 下载脚本每个文件前插守卫：`if ((Get-PSDrive E).Free/1GB -lt 15) { exit }`（守卫值 = 底 + 单文件余量）
- 转码类管线同样守卫目标盘（产物 ≈ 源大小，ABR 转码不省空间）
- 应急腾空间：`Clear-RecycleBin -DriveLetter X -Force`（网盘可重下的内容回收站无价值）

## 2. daemon 生命周期（hub）

- 长下载/转码用 `hub start`（pwsh -NoProfile -File script.ps1，pty=false，ready.log 匹配 "=== /"）
- **重启必须三步**：`hub stop` → `Stop-Process BaiduPCS-Go -Force` → `hub start`。hub stop/restart 只杀 pwsh，baidupcs-go 子进程孤儿续跑
- 看到两个 BaiduPCS-Go 进程先查 CommandLine：scoop shim（shims\baidupcs-go.exe）+ 真身（apps\current\BaiduPCS-Go.exe）是父子对 = 单下载，勿误判
- 限流（1-2MB/s）恢复 = 完整三步重启（断点续传零损失）；只 hub restart 无效（孤儿占连接）

## 3. 文件格式坑

- `.mp4.zst`/`.mkv.zst` = 纯 zstd 流 → 用 `7z x`（scoop 7zip），Win11 tar.exe 报 "Unrecognized archive format"
- `.tar.zst` 才能 `tar -xf`
...
- 有规则的按规则立即处理；无规则/超预算的写 `D:\处理询问.md`（编号 C1/C2...，每项给方案选项），等用户逐条回复
- 每批结束更新文档尾部"处理进度"段：完成项、空间现况（三盘 free）、卡点原因
- 数字必须实测核对后再写进文档（不猜）
