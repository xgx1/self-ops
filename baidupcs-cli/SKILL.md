---
name: baidupcs-cli
description: 使用百度网盘 CLI（BaiduPCS-Go）时查阅：scoop 安装、BDUSS 登录、自动化扫码登录（含弹窗/抓图陷阱）、SVIP 并发下载实测值、下载落盘结构、GBK 编码坑、秒传已失效
---

---
name: baidupcs-cli
description: 使用百度网盘 CLI（BaiduPCS-Go）时查阅：scoop 安装、BDUSS/Cookie 登录、自动化扫码登录流程、下载落盘结构、GBK 编码坑、命令速查、非 SVIP 并发限制、秒传已失效、备份档案窥探等实测教训
---

# BaiduPCS-Go CLI 使用教训

百度网盘命令行客户端（qjfoidnh fork v4.0.1）。本机已通过 scoop 安装并登录。以下教训区分「实测确认」与「[INFERENCE] 社区经验」。
...
- 登录态存 scoop persist 的 config，多账号用 `loglist`/`su` 管理
...
3. 全页截图发给用户会糊、扫不出——**必须局部截二维码元素或抓原图**，再用 PowerShell `System.Drawing` 以 NearestNeighbor 放大 3~4 倍给用户扫
   - 抓图用 run 作用域的 Node `fetch(src)` 写盘；**不要 `page.goto(二维码src)`**——会摧毁登录弹窗状态（导航回去后 QR img 元素没了，得重新点登录）
4. 打开图片给用户：`Start-Process` 走默认图片关联**可能无弹窗**（Win11 照片应用关联异常，实测踩过）。稳妥用法：`Start-Process rundll32 -ArgumentList '"C:\Program Files\Windows Photo Viewer\PhotoViewer.dll", ImageView_Fullscreen <图路径>' -PassThru`，1.5s 后查 `HasExited`，已退出再回落 `msedge.exe <图路径>`
5. run 里循环 `page.cookies()` 轮询 `BDUSS` 出现（2s/次，超时 170s），扫码确认后拿到 BDUSS+STOKEN
6. `BaiduPCS-Go login -bduss=.. -stoken=..` → `who` 验证。**BDUSS 含 `$` 字符，bash 里必须单引号包裹**
7. 收尾：`browser close`、删临时二维码图

注意：`STOKEN` 在 pan.baidu.com 域的 cookie 里也能拿到；二维码有效期几分钟。

## 下载/搜索行为（实测）

- **落盘目录是 `savedir\<uid>_<用户名>\`**，不是 savedir 根。例：savedir 设 `D:\Downloads`，实际落在 `D:\Downloads\1712471085_xxx\`
...
- **下载的文本文件可能是 GBK 编码**，`cat` 报 “stream did not contain valid UTF-8”，用 `iconv -f GBK -t UTF-8` 读
