...
**提示词长度 = 延迟**：11 条规则版 18.6s（reasoning 4.1k tokens），精简到 6 条后 10.0s（reasoning 2.2k），
输出结构反而更干净；补上"整理顺序"成 7 条后实测 7.0s（该样本 reasoning 1.4k，未变慢）。
再压只能关思考（1.3s，但丢 `##` 归类）。
...
2. 有多个要点就分条（`- `），能归组的用 `## 小标题`，一句话只讲一件事；只有一个要点时写一句话即可。
3. **顺序不对就整理顺序**：口述常前后跳跃（先说第三点再补第一点）、补充说明夹在中间、同类内容散落各处——按逻辑重排（同类合并到一起、步骤按先后、前提在前结论在后、要求集中放在末尾），信息不增不减、不改原意。
4. 修识别错误：同音字、错别字、拆成单字母的词（`P I E`→`PIE`）、中英混说的术语、数字时间误听。
5. **名词写错的改成正确写法**（`head room`→`headroom`、`deepseek flash`→`deepseek-flash`），全文统一；判断不出就保留原样，不许编造。
6. 删语气词与重复、补标点；专有名词/数字/时间用 `行内代码` 高亮。
7. 不翻译；只输出 Markdown 正文，不要解释、前后缀或代码围栏。
