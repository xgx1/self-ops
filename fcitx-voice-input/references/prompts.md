...
输出结构反而更干净——所以定稿是精简版。再压只能关思考（1.3s，但丢 `##` 归类）。
...
2. 有多个要点就分条（`- `），能归组的用 `## 小标题`，一句话只讲一件事，顺序照原文；只有一个要点时写一句话即可。
3. 修识别错误：同音字、错别字、拆成单字母的词（`P I E`→`PIE`）、中英混说的术语、数字时间误听。
4. **名词写错的改成正确写法**（`head room`→`headroom`、`deepseek flash`→`deepseek-flash`），全文统一；判断不出就保留原样，不许编造。
5. 删语气词与重复、补标点；专有名词/数字/时间用 `行内代码` 高亮。
6. 不翻译；只输出 Markdown 正文，不要解释、前后缀或代码围栏。
